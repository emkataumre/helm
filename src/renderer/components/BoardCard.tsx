...
export function BoardCard({ task, liveActivity, paused, resumable, blocked, waitingOn, planTitle, jailed, onClick, onRun, onDropIn, onStartFresh, onAbandon, onNewTerminal, onClearDeps, onOpenPlan }: {
    task: Task;
    liveActivity?: string;
    paused?: boolean;
    resumable?: boolean;
    ...
    jailed?: boolean;   // M13: the task's project runs jailed in Docker (skip-permissions behind the container wall)
    onClick?: () => void;
    onRun?: () => void;
    onDropIn?: () => void;
    onStartFresh?: () => void;
    ...
    return (
        <div
            ...
            {...verifyAttrs({ unit: "BoardCard", status: task.status, id: task.id, dropin: canDropIn, resumable: canResume, blocked: isBlocked, "waiting-on": waitingTitles || null, plan: planTitle || null, jail: jailed || null })}
            onClick={onClick}
            style={{ border: "1px solid #ccc", borderRadius: 8, padding: 10, marginBottom: 8, cursor: onClick ? "pointer" : "default" }}
        >
            <div>
                <b>{task.title}</b> {task.diffstat ? <code style={{ fontSize: 11 }}>{task.diffstat}</code> : null}
                {jailed ? <span title="Runs jailed in a Docker container (--dangerously-skip-permissions behind the container wall)" style={{ fontSize: 11, fontFamily: "ui-monospace, monospace", color: "#D97757", background: "#F0EEE6", border: "1px solid #E3DACC", borderRadius: 6, padding: "1px 6px", marginLeft: 6 }}>jailed</span> : null}
            </div>
            {planTitle ? (
                <button
