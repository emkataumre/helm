...
// Abandon. The deliberate hand-back trio lives in TaskDetail (HandbackActions).
export function BoardCard({ task, liveActivity, paused, onClick, onRun, onDropIn, onStartFresh, onAbandon }: {
    task: Task;
    liveActivity?: string;
    paused?: boolean;
    onClick?: () => void;
    ...
    return (
        <div
            ...
            {...verifyAttrs({ unit: "BoardCard", status: task.status, id: task.id, dropin: canDropIn })}
            onClick={onClick}
        ...
        >
            ...
            {canDropIn ? (
                <div style={{ marginTop: 6, display: "flex", gap: 6, flexWrap: "wrap" }}>
                    {onDropIn ? <button onClick={click(onDropIn)}>Drop in</button> : null}
