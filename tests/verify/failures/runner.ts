// tests/verify/failures/runner.ts
...
import { runFailuresInvariants, type InvariantResult } from "./invariants";
import { FAILURES_FIXTURES, type FailuresFixture, type ProbeFixture } from "./fixtures";

export type Verdict = "PASS" | "FAIL" | "BLOCKED" | "SKIP";

export async function buildRecording(fixture: FailuresFixture): Promise<FailuresRecording> {
    return fixture.probe ? fixture.recording : fixture.run();
}

export interface FailuresFixtureResult { fixture: string; probe: boolean; verdict: Verdict; checks: InvariantResult[]; notes: string[] }

export async function runFailuresFixture(fixture: FailuresFixture): Promise<FailuresFixtureResult> {
    const probe = Boolean(fixture.probe);
    let recording: FailuresRecording;
    try {
    ...
    const checks = runFailuresInvariants(recording);
...
export async function runAll(): Promise<FailuresFixtureResult[]> {
    if (FAILURES_FIXTURES.length === 0) return [{ fixture: "(none)", probe: false, verdict: "SKIP", checks: [], notes: ["no fixtures declared"] }];
    return Promise.all(FAILURES_FIXTURES.map(runFailuresFixture));
}
